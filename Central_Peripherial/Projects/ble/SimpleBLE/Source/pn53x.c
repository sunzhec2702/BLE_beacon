...
int
pn53x_transceive(struct nfc_device *pnd, const uint8 *pbtTx, const size_t szTx, uint8 *pbtRx, const size_t szRxLen, int timeout)
{
  bool mi = false;
  int res = 0;
...
int
pn53x_set_parameters(struct nfc_device *pnd, const uint8 ui8Parameter, const bool bEnable)
{
...
int
pn53x_set_tx_bits(struct nfc_device *pnd, const uint8 ui8Bits)
{
...
int
pn53x_wrap_frame(const uint8 *pbtTx, const size_t szTxBits, const uint8 *pbtTxPar,
                 uint8 *pbtFrame)
{
  uint8  btFrame;
  uint8  btData;
...
int
pn53x_unwrap_frame(const uint8 *pbtFrame, const size_t szFrameBits, uint8 *pbtRx, uint8 *pbtRxPar)
{
  uint8  btFrame;
  uint8  btData;
  uint8 uiBitPos;
...
int
pn53x_decode_target_data(const uint8 *pbtRawData, size_t szRawData, pn53x_type type, nfc_modulation_type nmt,
...
int
pn53x_write_register(struct nfc_device *pnd, const uint16 ui16RegisterAddress, const uint8 ui8SymbolMask, const uint8 ui8Value)
{
...
int
pn53x_initiator_transceive_bits(struct nfc_device *pnd, const uint8 *pbtTx, const size_t szTxBits,
...
int
pn53x_initiator_transceive_bytes(struct nfc_device *pnd, const uint8 *pbtTx, const size_t szTx, uint8 *pbtRx,
...
int
pn53x_target_init(struct nfc_device *pnd, nfc_target *pnt, uint8 *pbtRx, const size_t szRxLen, int timeout)
{
  pn53x_reset_settings(pnd);
...
int
pn53x_target_receive_bits(struct nfc_device *pnd, uint8 *pbtRx, const size_t szRxLen, uint8 *pbtRxPar)
{
  size_t szRxBits = 0;
...
int
pn53x_target_receive_bytes(struct nfc_device *pnd, uint8 *pbtRx, const size_t szRxLen, int timeout)
{
  uint8  abtCmd[1];
...
int
pn53x_target_send_bits(struct nfc_device *pnd, const uint8 *pbtTx, const size_t szTxBits, const uint8 *pbtTxPar)
{
  size_t  szFrameBits = 0;
  size_t  szFrameBytes = 0;
  uint8 ui8Bits = 0;
...
int
pn53x_target_send_bytes(struct nfc_device *pnd, const uint8 *pbtTx, const size_t szTx, int timeout)
{
  uint8  abtCmd[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  int res = 0;
...
int
pn53x_RFConfiguration__Various_timings(struct nfc_device *pnd, const uint8 fATR_RES_Timeout, const uint8 fRetryTimeout)
{
  uint8  abtCmd[] = {
    RFConfiguration,
    RFCI_TIMING,
...
int
pn53x_RFConfiguration__MaxRtyCOM(struct nfc_device *pnd, const uint8 MaxRtyCOM)
{
  uint8  abtCmd[] = {
    RFConfiguration,
    RFCI_RETRY_DATA,
...
int
pn53x_RFConfiguration__MaxRetries(struct nfc_device *pnd, const uint8 MxRtyATR, const uint8 MxRtyPSL, const uint8 MxRtyPassiveActivation)
{
...
int
pn53x_SetParameters(struct nfc_device *pnd, const uint8 ui8Value)
{
...
int
pn53x_InDeselect(struct nfc_device *pnd, const uint8 ui8Target)
{
  if (CHIP_DATA(pnd)->type == RCS360) {
...
int
pn53x_InRelease(struct nfc_device *pnd, const uint8 ui8Target)
{
  int res = 0;
  if (CHIP_DATA(pnd)->type == RCS360) {
    // We should do act here *only* if a target was previously selected
...
int
pn53x_check_ack_frame(struct nfc_device *pnd, const uint8 *pbtRxFrame, const size_t szRxFrameLen)
{
...
int
pn53x_check_error_frame(struct nfc_device *pnd, const uint8 *pbtRxFrame, const size_t szRxFrameLen)
{
 ...
 */
int
pn53x_build_frame(uint8 *pbtFrame, size_t *pszFrame, const uint8 *pbtData, const size_t szData)
{
...
pn53x_current_target_new(const struct nfc_device *pnd, const nfc_target *pnt)
{
  ...
  if (CHIP_DATA(pnd)->current_target) {
    osal_mem_free(CHIP_DATA(pnd)->current_target);
  }
...
{
  pnd->chip_data = osal_mem_alloc(sizeof(struct pn53x_data));
  if (!pnd->chip_data) {
    return NULL;
  }
  // Keep I/O functions
