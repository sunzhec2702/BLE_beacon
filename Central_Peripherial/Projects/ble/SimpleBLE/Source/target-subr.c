...
int
snprint_hex(char *dst, size_t size, const uint8 *pbtData, const size_t szBytes)
{
  size_t  szPos;
  size_t res = 0;
...
snprint_nfc_iso14443a_info(char *dst, size_t size, const nfc_iso14443a_info *pnai, bool verbose)
{
  ...
  if (verbose) {
    ...
    atqa = (((uint16)pnai->abtAtqa[0] & 0xff) << 8);
    atqa += (((uint16)pnai->abtAtqa[1] & 0xff));
    sak = ((uint8)pnai->btSak & 0xff);
